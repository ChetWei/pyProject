...
class Role(models.Model):
    """角色表"""
    name = models.CharField(max_length=64, unique=True)  # 不能重复
    menus = models.ManyToManyField("Menus",blank=True) #动态菜单
...
class CustomerInfo(models.Model):
    ...
    #关联自己，如果是转介绍(介绍人已经是学员，然后介绍别人过来学习)，需要填写转介绍人的信息，不是转介绍，默认空
    referral_form = models.ForeignKey('self',blank=True,null=True,verbose_name='转介绍',on_delete=models.CASCADE)
    #可以咨询多个课程
    consult_courses = models.ManyToManyField('Course',verbose_name='咨询课程')
    cousult_content = models.TextField('咨询内容')
